...
export default function ProductionSystemDetails(props) {
  const { getEventsByCropId, eventInfo, eventLoading, getEventByproductionSystemId } =
  ...
  useEffect(() => {
    const productionSystemId = props?.route?.params?.productionSystem?._id;
    (async () => {
      if (productionSystemId) {
        setCropEvents(await getEventByproductionSystemId(productionSystemId))
      }
    })()
  ...
  useEffect(() => {
    ...
    setProductionSystems(props?.route?.params?.productionSystem);
  ...
  var cropFields = [];
  for (var field of productionSystems?.fieldDetails || []) {
    let map = field.map || "";
    map = map.split(" ");
    map = map.map((m) => ({
  ...
  return (
    <SafeAreaView>
      ...
      <FarmerHeader
        ...
        title={productionSystems?.name}
        subtitle={
          (productionSystems?.name || "") +
          ", " +
          (productionSystems?.category || "")
        }
      />
      <TouchableOpacity
            ...
            rows={[
              {
                ...
                value: productionSystems.name,
                visible: !!productionSystems.name,
              },
              {
                title: i18n.t("farmer.cropsScreen.detailsTable.title1"),
                value: `${productionSystems?.fieldDetails?.[0]?.areaInAcres} Acres`,
                visible: !!productionSystems?.fieldDetails?.[0]?.areaInAcres,
              },
              {
                title: i18n.t("farmer.cropsScreen.detailsTable.title11"),
                value: productionSystems.category,
                visible: !!productionSystems.category,
              },
            ]}
          />
        </DetailsContainer>
            ...
            {(cropEvents || [])?.map((details, index) => {
              ...
              return (
                <EventCard
                  ...
                  details={{ ...details, isProductionSystem: true }}
                  crop={crop}
                  image={
                    details?.isCached ?
                      details?.image?.[0]?.uri
      ...
      >
        <CreateEvent
          ...
          details={props?.route?.params?.productionSystem}
